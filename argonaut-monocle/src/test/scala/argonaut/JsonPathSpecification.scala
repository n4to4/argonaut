...
object JsonPathSpecification extends Specification {
  ...
  "JsonPath" >> {
    ...
    "support parsing json using a codec" >> {
      root.cars.index(0).unsafeAs[Car].getOption(john) must_== Some(Car("fancy", 120, automatic = false))
    }
  }
  ...
  "JsonTraversalPath" >> {
    ...
    "support filtering by index of json array" >> {
      root.cars.filterByIndex(_ % 2 == 1).unsafeAs[Car].getAll(john) must_== List(Car("suv", 80, automatic = true))
    }
    ...
    "support an unsafe filtering by value" >> {
      root.cars.each.unsafeFilter(root.maxSpeed.int.asFold.exist(_ > 100)(_)).model.string.set("new")(john) must_== Json.obj(
        "first_name" -> "John".asJson,
        "last_name"  -> "Doe".asJson,
        "age"        -> 25.asJson,
        "address"    -> Json.obj(
